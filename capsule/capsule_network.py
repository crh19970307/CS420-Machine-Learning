...
class CapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=7200, in_channels=8,
                                           out_channels=16)
...
if __name__ == "__main__":
    ...
    def get_iterator(mode):
        dataset = My_MNIST(root='..', train=mode)
        # data = getattr(dataset, 'train_data' if mode else 'test_data')
        # labels = getattr(dataset, 'train_labels' if mode else 'test_labels')
        data = getattr(dataset, 'data')
        labels = getattr(dataset, 'label')
    ...
    def on_forward(state):
        meter_accuracy.add(state['output'].data, torch.ByteTensor(state['sample'][1]).long())
        confusion_meter.add(state['output'].data, torch.ByteTensor(state['sample'][1]).long())
